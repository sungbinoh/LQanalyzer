...
void AnalyzerCore::FillCLHist(histtype type, TString hist, snu::KEvent ev, std::vector<KLepton> Leptons, int N_electron, int N_muon, vector<snu::KJet> jets, std::vector<snu::KFatJet> fatjets,  double w, int nbjet){
  if(type==hnpairmm){
    map<TString, HNpairPlotsMM*>::iterator HNpairmmit = mapCLhistHNpairMM.find(hist);
    if(HNpairmmit !=mapCLhistHNpairMM.end()) HNpairmmit->second->Fill(ev, Leptons, N_electron, N_electron, jets, fatjets, w, nbjet);
    else {
      mapCLhistHNpairMM[hist] = new HNpairPlotsMM(hist);
      HNpairmmit = mapCLhistHNpairMM.find(hist);
      HNpairmmit->second->Fill(ev, Leptons, N_electron, N_electron, jets, fatjets, w, nbjet);
    }
  }
}
